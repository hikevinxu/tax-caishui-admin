			...
			<el-form label-width="130px" :model="formData" :rules="rules" ref="createPage">
				...
				<el-form-item label="落地页url：" prop="landpageUrl">
	          	...
	          	<el-form-item
	            	label="头图："
	            	prop="headImg"
	            	v-if="formData.packageName != '风险检测h5'"
	          	>
		            <el-upload
...
<script>
	...
	export default {
		data() {
			return {
				...
				rules: {
					...
					// headImg: [
					// 	{
					// 		required: true, message: '请添加头图', trigger: 'change'
					// 	}
					// ],
					// pageType: [
					// 	{
					// 		required: true, message: '请选择是否预注册', trigger: 'change'
					// 	}
					// ],
					// buttonRemark: [
					// 	{
		...
		methods: {
			getDetail() {
				ajax('api/channelPage/obtainDetail', 'post', {id: this.$route.query.id}).then(res => {
					if(res.code = 10001) {
			...
			handleSubmit() {
				ajax('api/channelPage/modify', 'post', this.formData).then(res => {
					if(res.code === 10001) {
						this.$message({
							type: 'success',
							message: '保存成功'
